...
const SaveToMMOButton = () => {
  const customization = useConfiguratorStore((state) => state.customization);
  
  const saveToMMO = async () => {
    // Get auth token from localStorage (assuming it's set by the parent)
    ...
    // Get username and race from form or assume defaults
    const username = prompt('Enter your username:');
    if (!username) return;
    
    const race = prompt('Choose race (human/elf/dwarf/voidborn):', 'human');
    if (!['human', 'elf', 'dwarf', 'voidborn'].includes(race)) {
      alert('Invalid race');
      return;
    }
    ...
    try {
      ...
      if (data.error) {
        ...
        alert('Character saved to Dominion!');
        // Redirect to game or something
        window.parent.postMessage({ type: 'characterSaved', profile: data }, '*');
      }
    } catch (error) {
